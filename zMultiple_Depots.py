...
@author: Markus Bozau
muss fuer abstossende Punkte angepasst werden da dafuer fehlerhaft
momentan fuer alle depots insgeasammt nur die maximale Distanz beruecksichtigt da evtuell die summe besser
dafuer muss einiges umgebaut werden
...
class WriteFiles(object):
    ...
    def __init__(self,approx_level_integer,desirable_knot_dict,obnoxious_knot_dict,desirable_goal,obnoxious_goal,fact_number=1): #goals 1 fermat weber  bzw summe        2 centerproblem
        ...
        self.gurobi_solver="C:\\Users\\Markus Bozau\\Desktop\\ampl_mswin64_WS19\\gurobi.exe"
    ...
    def Approx_Constr(self):
        ...
        yield("subject to MIN_12{k in KNOTS,i in Levels}: z_2[k,i]>=q[i] * z_1[k,i-1] - y[k,i]*M ;" )       
        yield("subject to MIN_13{k in KNOTS,i in Levels}: z_2[k,i]<=q[i] * z_2[k,i-1];" )   
        yield("subject to MIN_14{k in KNOTS,i in Levels}: z_2[k,i]>=q[i] * z_2[k,i-1] - (1 - y[k,i])*M ;" )  
        ...
        #yield("subject to MIN_16{k in KNOTS,i in Levels}: z_1[k,i-1] - z_2[k,i-1] >= 0 - (1 - y[k,i])*M ;") #vorruebergehend aus glaube ueflue wegen ABS 11 bis 14
    ...
    def Coord_Diffs(self):
        yield("subject to A_Diff{k in KNOTS,d in DEPOTS}: a_d[k,d] = a_coord[k] - new_a[d] ;")  #Approximierter Wert fuer euklidische Entfernung  
        yield("subject to B_Diff{k in KNOTS,d in DEPOTS}: b_d[k,d] = b_coord[k] - new_b[d] ;")
        yield("subject to A_Diff_up{k in KNOTS,d in DEPOTS}: a[k]<=a_d[k,d]+(1-p[k,d])*2*M;")
        yield("subject to A_Diff_low{k in KNOTS,d in DEPOTS}: a[k]>=a_d[k,d]-(1-p[k,d])*2*M;")
        yield("subject to B_Diff_up{k in KNOTS,d in DEPOTS}: b[k]<=b_d[k,d]+(1-p[k,d])*2*M;")
        yield("subject to B_Diff_low{k in KNOTS,d in DEPOTS}: b[k]>=b_d[k,d]-(1-p[k,d])*2*M;")
        yield("subject to One_Depot{k in KNOTS}:sum{d in DEPOTS}p[k,d]=1;")
        yield("\n")
    ...
    def Defs(self):
        ...
        yield("set DEPOTS:= 1 .. depot_number; ")
        ...
        yield("param M:= 2000;")
        yield("var new_a{DEPOTS} >=0 <=1000;")
        yield("var new_b{DEPOTS} >=0 <=1000;")
        yield("var a{KNOTS} ;")
        yield("var b{KNOTS} ;")
        yield("var a_d{KNOTS,DEPOTS} ;")
        yield("var b_d{KNOTS,DEPOTS} ;")
        yield("var p{KNOTS,DEPOTS} binary;")
    ...
    def StartSolver(self):
        ...
        solve_process=subprocess.Popen([self.gurobi_solver,"TimeLimit=1000","MIPGap=0.0001","ResultFile="+self.o_filename,self.lp_filename,"MIPGap= 0.8"])#"LogFile="+os.getcwd()+"\sudo.log","ResultFile="+os.getcwd()+"\sudo.sol",self.lp_filename,"MIPGap= 0.8"])#,"NodefileStart=4","Threads =3"])
    ...
    def DatFileContent(self):        
        ...
        yield("param depot_number := %s ;" %(self.fact_number))
        a_string=""
        b_string=""
    ...
    def GetSolution(self):
        ...
        x_y_list=self.FormatSolution(whole_solution)
    ...
    def FormatSolution(self,whole_solution):
        ...
        for sol in whole_solution:
            ...
            if len(sol_1)>1:
                ...
                var_value.strip("\n")
                if var_name=="new_a":
                    x_dict[var_ind]=var_value
                elif var_name=="new_b":
                    y_dict[var_ind]=var_value
            #sol[0]=sol[0].strip('(')
            #sol[0]=sol[0].strip(')')
            #sol[0]=sol[0].strip('Z')
            #sol[0]=sol[0].split(',')
        for key in x_dict:
            x_y_list.append((int(float(x_dict[key])),int(float(y_dict[key]))))
        return x_y_list      
...
def SolveNewProblem(desirable_dict,obnoxious_dict):
    ...
    Wf=WriteFiles(4,desirable_dict,obnoxious_dict,2,2,5) 
    Wf.WriteModFile() 
    Wf.WriteDatFile()  
    Wf.WriteRunFile()   
    Wf.StartSolver()
    x_y_list=Wf.GetSolution()
    return x_y_list
...
desirable_dict,obnoxious_dict=InstanceGenerator(40, 0, max_x, max_y)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if 475<=event.pos[0]<=525 and 0<=event.pos[1]<=50:
                x_y_list=SolveNewProblem(desirable_dict,obnoxious_dict)
                print(x_y_list)
                for x_y in x_y_list:
                    pygame.draw.circle(screen,yellow,x_y,3,2)
                    pygame.display.flip()
